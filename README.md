...
| **timeline** | {`userId`: (**optional**) string - If we pass a **userId** then we will retrieve that users data, if not we will retrieve **/me** users data., `from`: (**optional**) string - A timestamp string. If we specify a from property, only the results that are BEFORE this timestamp will be returned. } |  Retrieves the current users (or another users) timeline data| {`data`: Object, `error`: Object} |

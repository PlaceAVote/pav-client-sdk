...
export function setEnvironmentIsDev(isDevEnvironment = false){
...
*/
export async function  _fetch(url, method, data, responseCodes, additionalHeaderItems = null){
  console.log("Http Request to: "+url);
  var response;
  try {
    response = await fetch(
      url,
      this.prepareFetchProperties(method, data, (data&&data.token)?data.token:null, additionalHeaderItems)
    );
  } catch(error) {
...
export function prepareFetchProperties(method, body, token, additionalHeaderItems){
  var reqOpts = {
    method: method,
    headers:{},
    body:null
  ...
  if(!!additionalHeaderItems){
    ...
    console.log(JSON.stringify(reqOpts.headers)+ "AFTER");
  }

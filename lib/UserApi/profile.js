 ...
 *
 *  {
 * 	accessToken: "AU389FGUGBA9GBAGWTHISISATOKEN91R89F", 	//Optional - Optional ONLY if you have already logged in and the token is already set in PavUtils.
...
export async function profile(data) {
	...
	if(data && data.userId){	//if the data object contains a userId 
		...
		'GET', null, {accept: [200,201], reject:[400,401,404]}, {user_id: data.userId});
		...
		'GET',null, {accept: [200,201], reject:[400,401,404]});
	}
	return response;
}
...
export async function timeline(data) {
	...
	if(data && data.userId){	//if the data object contains a userId 
		response = await PavUtils._fetch(   
		PavUtils.getBaseUrl()+PavUtils.sPrintf(ENDPOINTS.USER.TIMELINE,data.userId),	//then form the url to contain that userId
		'GET', null, {accept: [200,201], reject:[400,401,404]}, {user_id: data.userId});
	}else{						//if we got NO userId	
		response = await PavUtils._fetch(   
		PavUtils.getBaseUrl()+PavUtils.sPrintf(ENDPOINTS.USER.TIMELINE,'me'),	//then form the url to contain the 'me' keyword
		'GET',null, {accept: [200,201], reject:[400,401,404]});
	}
	return response;
}

...
export async function login(data) {
  ...
  let response = await PavUtils._fetch(   //call the endpoint
      API_BASE_URL+ENDPOINTS.USER.LOGIN_EMAIL,
      'POST', {
      email: data.email,
      password: data.password
  });
...
export async function loginFacebook(data) {
  ...
  PavUtils.checkThatDataExistsAndNotNull([  //check that the data provided is valid
    {data: data, errorSpec: "not be null or undefined"},
    {data: data.fbUserId, errorSpec: "contain a facebook user id within the fbUserId property"},
    {data: data.fbAccessToken, errorSpec: "contain a facebook access token within the fbAccessToken property"}
    ], "loginFacebook");

  let response = await PavUtils._fetch(   //call the endpoint
      API_BASE_URL+ENDPOINTS.USER.LOGIN_FACEBOOK,
      'POST', {
      id: data.fbUserId,
...
export async function validate(data) {
  ...
  let response = await PavUtils._fetch(   //call the endpoint
      API_BASE_URL+ENDPOINTS.USER.VALIDATE,
      'POST', {
      email: data.email
  });
  return response;
